...
<Warning>
  이 필드들을 설정하는 API 나 VoiceML verb 는 없습니다. 전환을 **지시**하는 것과 그 결과가
  **기록**되는 것은 다릅니다. 전환은 에이전트가 통화 중에 실행하고, 위 필드는 그렇게 실행된
  전환의 결과가 사후에 채워지는 **읽기 전용 관측 필드**입니다. 전환이 없었던 통화는 전부
  `null` 입니다.
</Warning>
...
| `transferHangupCauseQ850` | number \| null | 전환 leg 의 통신망 Q.850 cause 입니다. 16은 정상해제, 17은 통화중, 19는 무응답, 21은 거절, 38은 망장애입니다. |
...
1. 끝난 leg 만 후보입니다. `initiated`, `ringing`, `connected`, `interrupted` 는 제외됩니다.
2. `completed` 인 leg 를 우선합니다.
3. 그래도 여럿이면 `sequence` 가 가장 큰 leg 입니다.
...
| `mode` | string \| null | `blind`(바로 넘김), `warm`(대상에게 안내 멘트 후 연결), `refer`와 `refer-attended`(SIP 단말이 건 REFER 전환)입니다. |
...
<Warning>
  ...
  `transfers[].billableDuration` 이고, 한 통화에서 `billable` 이 `true` 인 leg 는 최대 한 건입니다.
  전환 요금을 계산하실 때 `duration` 을 합산하지 마세요.
</Warning>
...
`transfer` 는 기본 이벤트에 포함되지 않습니다. 받으려면 기본 네 개와 함께 직접 나열해야 합니다.
