---
title: 메시지와 알림톡
description: 하나의 엔드포인트로 SMS·LMS·MMS·카카오 알림톡을 발송합니다. 문자 타입 선택, 알림톡 채널·템플릿 조회, 발송 결과와 webhook을 안내합니다.
slug: messages
---

ClawOps는 하나의 엔드포인트로 SMS·LMS·MMS·카카오 알림톡을 발송합니다. 문자에서 `Type`을 생략하면 본문 길이와 제목·첨부 유무에 맞는 유형을 플랫폼이 자동으로 선택합니다. 알림톡은 `Kakao`를 실어 보냅니다.
...
<Info>
  이 가이드에서는 SMS·LMS·MMS·카카오 알림톡을 다룹니다. iMessage와 RCS는 지원하지 않습니다.
</Info>
...
| | SMS | LMS | MMS |
| --- | --- | --- | --- |
| 본문 한도 | EUC-KR 90byte | 2,000자 | 2,000자 |
| 제목 (`Subject`) | 불가 | 가능 | 가능 |
| 이미지 첨부 | 불가 | 불가 | 최대 3장 |

한도의 기준이 서로 다릅니다. SMS는 EUC-KR 바이트로, LMS와 MMS는 글자 수로 셉니다. ASCII 영문·숫자는 1byte, 한글 등 비 ASCII 문자는 안전하게 2byte로 계산하므로 한글로만 작성하면 SMS에 45자까지 들어갑니다. EUC-KR로 표현할 수 없는 문자는 통신사 구간에서 `?`로 치환될 수 있습니다.

<Tip>
  본문 길이가 런타임에 달라진다면 `Type`을 생략하세요. 90byte 이하는 SMS, 초과분은 LMS로 자동 발송됩니다. `sms`를 명시한 본문이 90byte를 넘으면 잘린 채 발송되지 않도록 `400 body_too_long`을 반환합니다.
</Tip>
...
| `Body` | string | **문자 발송 시 필수.** SMS는 EUC-KR 90byte, LMS와 MMS는 2,000자까지 허용합니다. |
| `Type` | string | 선택 사항입니다. 생략하면 본문이 EUC-KR 90byte를 넘을 때 LMS, 첨부가 있으면 MMS, 그 외에는 SMS로 자동 판별합니다. 특정 유형을 강제할 때만 `sms`, `lms`, `mms`를 지정합니다. |
...
알림톡은 **ClawOps 리소스 ID**로 발송합니다. API만으로 채널과
템플릿을 조회한 뒤 그 ID를 발송 요청에 사용하세요.
...
채널 목록의 `data[].id`를 템플릿 조회와 알림톡 발송에 사용합니다.
...
`sendable`이 `true`인 템플릿을 고르고, `variables`에 나온 모든 변수를 채워 발송합니다.
...
<Info>
  알림톡에는 `Body`를 넣지 않습니다. 본문과 버튼은 검수된 템플릿이 정하고,
  발송 시점에는 `Variables`만 채울 수 있습니다.
</Info>
...
```json title="업무 규칙 위반"
{ "error": "SMS Body는 90byte(EUC-KR, 한글 45자)를 초과할 수 없습니다. Type을 생략하면 LMS로 자동 발송됩니다", "code": "body_too_long" }
```
...
| `400` | `body_too_long` | `SMS Body는 90byte(EUC-KR, 한글 45자)를 초과할 수 없습니다. Type을 생략하면 LMS로 자동 발송됩니다` | 명시적으로 `sms`를 선택한 본문이 SMS 한도를 넘었습니다. `Type`을 생략해 자동 판별하거나 `lms`로 보내세요. |
