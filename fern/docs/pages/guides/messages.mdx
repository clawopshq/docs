---
title: 문자 메시지
description: SMS·LMS·MMS 발송과 문자 타입 선택, 발송 결과 및 webhook을 안내합니다.
slug: messages
---

ClawOps는 하나의 엔드포인트로 SMS·LMS·MMS를 발송합니다. `Type`을 생략하면 본문 길이와 제목·첨부 유무에 맞는 유형을 플랫폼이 자동으로 선택합니다.
...
<Info>
  이 가이드에서는 SMS·LMS·MMS 문자 메시지만 다룹니다. 카카오 알림톡은 [카카오 알림톡 가이드](/kakao-alimtalk)를 확인하세요. iMessage와 RCS는 지원하지 않습니다.
</Info>
...
| `Body` | string | **필수.** SMS는 EUC-KR 90byte, LMS와 MMS는 2,000자까지 허용합니다. |
...
- [전화번호 가이드](/numbers)에서 문자 발신번호를 준비하세요.
- 카카오 메시지는 [알림톡 보내기](/kakao-alimtalk)를 참고하세요.
- 전체 요청·응답 필드는 [Messages API](/api-reference/claw-ops-api/messages/list-messages)에서 확인하세요.
