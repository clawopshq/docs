...
<Warning>
  엑셀이나 CSV에서 명단을 뽑으면 앞자리 `0`이 떨어져 `1012345678`이 되기 쉽습니다. 이런 값은 `400`으로 거절됩니다. 숫자가 아니라 텍스트 서식으로 내보내거나, 발송 전에 `0`을 복원하세요.
</Warning>
...
- **070 번호 발급.** 가장 일반적인 경로입니다.
- **대표번호 발급.** `representative_number` 부가서비스가 필요합니다.
- **모바일 게이트웨이 회선 등록.** 전용 장비에 꽂은 회선의 번호가 인증 문자를 거쳐 등록됩니다. 장비 없이 임의의 휴대폰 번호를 발신번호로 등록하는 경로는 없습니다.

<Warning>
  **계정에 등록된 번호라고 해서 문자가 나가는 것은 아닙니다.** 문자 발신번호는 통신사 쪽에 따로 사전등록되어야 하며 이는 음성 발신등록과 별개 절차입니다. 등록이 안 된 번호로 보내면 API는 `201`로 정상 접수하고 통신사가 뒤늦게 거절합니다. 결과는 `message.failed`로 오지만 **사유는 전달되지 않으므로** 원인 없이 전량 실패하는 것처럼 보입니다. 새 번호를 받은 직후 문자가 전부 실패한다면 이 경우를 먼저 의심하세요.
</Warning>
