---
title: API Reference
description: ClawOps REST API 인증, 기본 URL과 공통 규칙입니다.
slug: api-reference
---

# API Reference

ClawOps REST API는 HTTPS와 JSON을 사용합니다.

## Base URL

```text
https://api.claw-ops.com
```

## 인증

모든 요청의 `Authorization` 헤더에 API 키를 Bearer 토큰으로 전달합니다.

```bash
curl "https://api.claw-ops.com/v1/accounts/$CLAWOPS_ACCOUNT_ID/calls" \
  -H "Authorization: Bearer $CLAWOPS_API_KEY"
```

<Warning>
  API 키는 서버 측 비밀 저장소에서 관리하고 Git 저장소, 브라우저 코드, 로그에 기록하지 마세요.
</Warning>

왼쪽 탐색 메뉴에서 OpenAPI 스펙으로 자동 생성된 전체 엔드포인트와 요청·응답 스키마를 확인할 수 있습니다.

## 작업별 가이드

- [에이전트](/agents) — 음성 에이전트 생성과 전화번호 연결
- [전화번호](/numbers) — 번호 발급, 라우팅, 대표번호와 반납
- [문자 메시지](/messages) — SMS·LMS·MMS 발송과 결과 확인
- [카카오 비즈니스 시작하기](/kakao-business-start) — 비즈니스 채널 준비와 연결
- [알림톡 보내기](/kakao-alimtalk) — 템플릿 조회와 알림톡 발송
- [통화 전환 결과](/call-transfers) — 전환 결과 조회와 callback 처리
